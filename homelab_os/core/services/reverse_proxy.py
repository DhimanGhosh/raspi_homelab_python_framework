...
from homelab_os.core.services.app_catalog import AppCatalog, load_app_catalog


class ReverseProxyService:
    def __init__(self, settings: Settings) -> None:
        self.settings = settings
        self.catalog: AppCatalog = load_app_catalog(str(settings.app_catalog_file))
    ...
    def _app_meta(self, plugin_id: str) -> dict | None:
        return self.catalog.get_app(plugin_id)

    def has_public_route(self, plugin_id: str) -> bool:
        if plugin_id == 'control-center':
            return True
        app = self._app_meta(plugin_id)
        return bool(app and app.get('public_port') is not None)

    def public_port_for_plugin(self, plugin_id: str) -> int:
        if plugin_id == 'control-center':
            return self.settings.control_center_public_port
        app = self._app_meta(plugin_id)
        if not app or app.get('public_port') is None:
    ...
    def public_url_for_plugin(self, plugin_id: str) -> str | None:
        ...
        suffix = '/' if plugin_id == 'control-center' else (self._app_meta(plugin_id) or {}).get('entrypoint_path', '/')
    ...
    def remove_plugin_route(self, plugin_id: str) -> None:
        if plugin_id == 'control-center' or not self.has_public_route(plugin_id):
            return
