let state = { apps: [], jobs: [], total_bundles: 0, current_version: '3', tailscale_fqdn: '', notifications: [] };
let appSearchTerm = '';
...
function matchesSearch(text) {
  return !appSearchTerm || String(text || '').toLowerCase().includes(appSearchTerm);
}
...
function renderHeader() {
  ...
  document.getElementById('dockerRoot').textContent = `Docker root: ${state.docker_root || '-'}`;
  setMetric('nasUsage', state.nas_usage || {});
  setMetric('homelabUsage', state.homelab_usage || {});
  setMetric('rootUsage', state.root_usage || {});
}
...
function appCard(app) {
  ...
  const openBtn = canOpen
    ? `<a class="btn" href="${esc(app.public_url)}" target="_blank" rel="noreferrer">Open</a>`
    : '';

  const primaryInstallOrUpdateBtn = installed
    ? (updateAvailable && latestBundle
      ? `<button class="btn" data-update-app="${esc(app.id)}" data-bundle-filename="${esc(latestBundle)}">Update</button>`
      : '')
    : (latestBundle
      ? `<button class="btn" data-install-bundle="${esc(app.id)}" data-bundle-filename="${esc(latestBundle)}">Install</button>`
      : '');

  const actionBtns = installed && app.id !== 'control-center'
    ? `${primaryInstallOrUpdateBtn}${status === 'running'
      ? `<button class="secondary" data-plugin-action="stop" data-plugin-id="${esc(app.id)}">Stop</button>`
  ...
  const bundleRows = (app.bundles || []).length
    ? app.bundles.map((b) => `
        <div class="bundle-row">
          <div class="muted small">
            ${esc(b.filename)}
            ${b.version ? `<div class="tiny muted">v${esc(b.version)}</div>` : ''}
          </div>
          <div class="bundle-actions">
            <button class="secondary" data-install-bundle="${esc(app.id)}" data-bundle-filename="${esc(b.filename)}">Install</button>
            <button class="danger-soft" data-delete-bundle="${esc(b.filename)}">Delete</button>
          </div>
        </div>`).join('')
    : '<div class="muted small">No bundles</div>';

  return `
    <div class="app-card">
      <div class="app-head">
        <div class="app-title">${esc(app.name)}</div>
        <span class="status-pill ${installed ? 'status-installed' : 'status-missing'}">${installed ? esc(app.installed_version || 'installed') : 'not installed'}</span>
      </div>
      ...
      <details class="bundles-block" data-app-id="${esc(app.id)}" ${isOpen ? 'open' : ''}>
        <summary>Bundles (${(app.bundles || []).length})</summary>
        ${bundleRows}
      </details>
    </div>`;
}
...
function renderJobs() {
  ...
  document.querySelectorAll('[data-show-log]').forEach((el) => {
    el.onclick = (e) => { e.stopPropagation(); openJobLog(el.dataset.showLog); };
  });
}

async function refreshSummary() {
  const { data } = await apiJson('/api/control-center/summary');
  state = Object.assign(state, data || {});
  if (openBundleId && !(state.apps || []).some((app) => app.id === openBundleId)) {
    openBundleId = null;
  }
  renderHeader();
  renderNotifications();
...
async function refreshSelectedLog() {
  ...
  const { data } = await apiJson('/api/jobs/' + encodeURIComponent(selectedJobId) + '/logs');
...
async function pluginAction(pluginId, action) {
  await apiJson('/api/control-center/plugins/' + encodeURIComponent(pluginId) + '/' + encodeURIComponent(action), { method: 'POST' });
  await refreshSummary();
}

async function installBundle(appId, filename) {
  await apiJson('/api/control-center/apps/' + encodeURIComponent(appId) + '/install-bundle/' + encodeURIComponent(filename), { method: 'POST' });
  await refreshSummary();
}

async function deleteBundle(filename) {
  await apiJson('/api/control-center/bundles/' + encodeURIComponent(filename) + '/delete', { method: 'POST' });
  await refreshSummary();
}
...
function wireAppButtons() {
  ...
  document.querySelectorAll('[data-install-bundle]').forEach((btn) => {
    btn.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
      installBundle(btn.dataset.installBundle, btn.dataset.bundleFilename).catch((err) => alert(String(err)));
    };
  });
  document.querySelectorAll('[data-delete-bundle]').forEach((btn) => {
    btn.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
      deleteBundle(btn.dataset.deleteBundle).catch((err) => alert(String(err)));
    };
  });
  document.querySelectorAll('[data-update-app]').forEach((btn) => {
    btn.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
...
function wireBundleDetailsBehavior() {
  const blocks = [...document.querySelectorAll('.bundles-block')];
  blocks.forEach((details) => {
    const appId = details.dataset.appId;
    details.open = openBundleId === appId;
...
document.getElementById('clearLogBtn')?.addEventListener('click', () => { document.getElementById('logBox').textContent = ''; selectedJobId = null; document.getElementById('logCard').style.display = 'none'; });
document.getElementById('hideLogBtn')?.addEventListener('click', () => { document.getElementById('logCard').style.display = 'none'; selectedJobId = null; });
document.getElementById('clearCompletedBtn')?.addEventListener('click', async () => { await apiJson('/api/control-center/jobs/clear-completed', { method: 'POST' }); await refreshSummary(); });
document.getElementById('clearAllBtn')?.addEventListener('click', async () => { if (!confirm('Clear all install queue jobs?')) return; await apiJson('/api/control-center/jobs/clear-all', { method: 'POST' }); await refreshSummary(); });
document.getElementById('appSearchInput')?.addEventListener('input', (e) => { appSearchTerm = String(e.target.value || '').trim().toLowerCase(); document.getElementById('clearAppSearchBtn').style.display = appSearchTerm ? 'inline-flex' : 'none'; renderApps(); });
document.getElementById('clearAppSearchBtn')?.addEventListener('click', () => { appSearchTerm = ''; const i = document.getElementById('appSearchInput'); i.value = ''; document.getElementById('clearAppSearchBtn').style.display = 'none'; renderApps(); });
